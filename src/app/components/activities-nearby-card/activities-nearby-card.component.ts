...
@Component({
  ...
  standalone: true,
  imports: [MatToolbarModule, NgIf, MapComponent],
...
export class ActivitiesNearbyCardComponent implements OnInit {
  ...
  constructor(private router: Router) {
    ...
    this.activitySelected =
      this.activityService.onGetDefaultVoiageActivitiesList();
  }
  ...
  checkboxChange(placeNearby: IPlaceSearchResult, event: any): void {
    ...
    if (event.target.checked) {
      ...
      this.notifyService.onCustomSimpleNotify(
        "Added to the list",
        "The activity has been added to the list",
      );
    } else {
  ...
  activityFilterInfo(placeNearby: IPlaceSearchResult): IActivity {
    const activityNearby: IActivity =
      this.activityService.onGetDefaultVoiageActivities();
