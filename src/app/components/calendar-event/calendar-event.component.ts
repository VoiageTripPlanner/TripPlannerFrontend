...
export class CalendarEventComponent {
  ...
  constructor(
    ...
    this.eventNgmodel = this.isEdit
      ? data.calendarEvent
      : this.calendarEventService.onGetDefaultCalendarEvent();
  }
    ...
    if (form.valid) {
      if (this.isEdit) {
        this.calendarEventService
          .updateCalendarEvent(this.eventNgmodel)
          .subscribe(() => this.notifyService.onSuccess());
      } else {
        this.eventNgmodel.creation_responsible =
          this.userInformation.getUserId();
        this.calendarEventService
          .saveCalendarEvent(this.eventNgmodel)
          .subscribe(() => this.notifyService.onSuccess());
      }
    } else {
  ...
  deleteCalendarEvent() {
    this.notifyService.onDeleteConfirmation().then((result) => {
      if (result.value) {
        this.calendarEventService
          .deleteCalendarEvent(this.eventNgmodel)
          .subscribe(() => this.notifyService.onSuccess());
      }
    });
    this.onClose();
