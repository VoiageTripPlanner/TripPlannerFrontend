...
import {
  IFoodBusiness,
  IYelpApiSearchParams,
} from "../../../interfaces/yelp-food-response.interface";
...
export class FoodCardComponent {
  ...
  checkboxChange(yelpFood: IFoodBusiness, event: any): void {
    ...
    if (event.target.checked) {
      ...
      this.notifyService.onCustomSimpleNotify(
        "Added to the list",
        "The restaurant has been added to the list",
      );
    } else {
      this.foodSelectedOption.yelpId
        ? this.foodService.removeItem(this.foodSelectedOption.yelpId)
        : 0;
      this.notifyService.onCustomSimpleNotify(
        "Removed from the list",
        "The restaurant has been removed of the list",
      );
    }
  }

  foodFilterInfo(yelpFood: IFoodBusiness): IVoiageRestaurant {
    let locationMark: ILocationMark =
      this.locationMark.onGetDefaultVoiageLocationMark();
    ...
    this.foodSelectedOption.locationMark =
      locationMark || this.locationMark.onGetDefaultVoiageLocationMark();

    //Datos que no persisten en la BD
    this.foodSelectedOption.yelpId = yelpFood.id || " ";
    this.foodSelectedOption.restaurantImage =
      yelpFood.image_url || "./assets/img/No_image_available.png";
    this.foodSelectedOption.addressLocation =
      yelpFood?.location?.display_address?.join(", ") || " ";
    this.foodSelectedOption.locationCityCountry =
      yelpFood?.location?.city + ", " + yelpFood?.location?.country || " ";

    return this.foodSelectedOption;
  }

  assingfoodLocalsNerby(yelpFoodResponseList: IFoodBusiness[]): void {
    this.foodLocalsNerby = yelpFoodResponseList.map(
      (foodBusiness: IFoodBusiness) => ({
        ...
        location: new google.maps.LatLng(
          foodBusiness.coordinates?.latitude || 0,
          foodBusiness.coordinates?.longitude || 0,
        ),
      }),
    );
    debugger;
  ...
  viewInMap(yelpFood: IFoodBusiness) {
    const place: IPlaceSearchResult = {
      ...
      location: new google.maps.LatLng(
        yelpFood.coordinates?.latitude || 0,
        yelpFood.coordinates?.longitude || 0,
      ),
    };
