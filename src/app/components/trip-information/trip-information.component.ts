...
export class TripInformationComponent implements OnInit {
  ...
  constructor(private router: Router) {
    ...
    this.foodSelectedlist = this.foodSelectedlist.filter(
      (food) => food.name !== "",
    );
    this.activitiesSelectedList = this.activitiesService.getActivities();
    this.activitiesSelectedList = this.activitiesSelectedList.filter(
          ...
          if (result.isConfirmed) {
            ...
            this.tripService.saveTrip(this.tripNgModel).subscribe(() => {
              this.notifyService.onCustomSimpleNotify(
                "Trip Saved",
                "The trip information has been saved successfully",
              );
            });
