...
export class ActivitiesNearbyComponent {
  ...
  constructor(private router: Router) {
    this.activitiesPriceEstimate =
      this.openAIService.onGetDefaultAISuggestion();
    this.storedFormData = this.tripService.getFormData()
      ? this.tripService.getFormData()
      : this.tripService.onGetDefaultTripForm();
  }
  ...
  priceEstimation() {
    let storedActivitiesData: IActivity[] =
      this.activitiesService.getActivities().length > 0
        ? this.activitiesService.getActivities()
        : this.activitiesService.onGetDefaultVoiageActivitiesList();

    //hacer el filtro de actividades vacias
    storedActivitiesData = storedActivitiesData.filter(
      (food) => food.name !== "",
    );
    ...
    this.openAIService
      .getPriceEstimate(this.activitiesPriceEstimate)
      .subscribe({
        next: (response) => {
        ...
        error: (err) => {
          ...
          this.notifyService.onCustomErrorNotify(
            "No price estimated",
            "Could not estimate the price of the activities",
          );
        },
        complete: () => {
  ...
  activitiesToEstimate(storedActivitiesData: IActivity[]) {
    storedActivitiesData.forEach((restaurant) => {
      let restaurantEstimate: IActivityEstimate =
        this.openAIService.onGetDefaultActivityEstimate();
