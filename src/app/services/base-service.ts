...
export class BaseService<T> {
  ...
  public findAllByUserAndPage(
    userId: number,
    page: number,
    size: number,
  ): Observable<IResponse<IPagination<T>>> {
    return this.http.get<IResponse<IPagination<T>>>(
      `${this.source}/pagination/${userId}`,
      { params: { page, size } },
    );
  }
  ...
  public findAllDetailed(s: string = ""): Observable<IResponse<T[]>> {
    return this.http.get<IResponse<T[]>>(this.source + "/userDetailed", {
      params: { s },
    });
  }
